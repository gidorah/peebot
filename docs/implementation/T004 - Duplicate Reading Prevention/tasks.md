...
- [x] **Step 3**: Add `ignore_conflicts=True` to the bulk insert in the ingestion command.
...
- [x] **Step 4**: Add constraint enforcement test.
    ...
    - *Note*: Replaced the old `test_composite_uniqueness_constraint` (which tested the PK and never fired on real data) with `test_unique_channel_timestamp_constraint`.

- [x] **Step 5**: Add duplicate-skip test for bulk insert.
    - *File*: `apps/telemetry_storage/tests/test_repositories.py`
    - *Task*: Add a test that calls `abulk_create(..., ignore_conflicts=True)` with two readings sharing the same `(channel, timestamp)`. Assert only one row is persisted and no exception is raised.
    - *Verification*: `just test` passes. ✓
    - *Note*: Also fixed `test_repository_bulk_create_async` to use distinct timestamps per reading (the old test was accidentally passing under the vacuous `(id, timestamp)` constraint).

- [x] **Step 6**: Add restart-simulation test for the ingestion command.
    - *File*: `apps/telemetry_ingestion/tests/test_deduplication.py` (new)
...
- [x] **Step 7**: Mark roadmap item complete.
    - *File*: `docs/system-solution/main-tasks.md`
    - *Task*: Changed `- [ ]` to `- [x]` and added completion date 2026-02-18. ✓
