...
class InjectTelemetryView(APIView):
    ...
    def post(self, request: Request, *args: object, **kwargs: object) -> Response:
        """
        Handle POST requests to inject a telemetry reading for a channel identified by a public PUI.
        
        Validates the request body as a ManualInjectionPayload, looks up the TelemetryChannel using the payload's `pui`, creates a telemetry reading, and returns the serialized reading. If validation fails, returns a 400 response with validation errors. If the channel is not found, returns a 404 response with a detail message.
        
        Parameters:
            request (rest_framework.request.Request): Incoming HTTP request containing the payload.
        
        Returns:
            dict: Serialized telemetry reading data on success.
        """
        try:
