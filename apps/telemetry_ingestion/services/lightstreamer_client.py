...
from lightstreamer.client.ls_python_client_haxe import com_lightstreamer_client_internal_update_ItemUpdateBase as ItemUpdate
...
class SubListener(SubscriptionListener):  # type: ignore[misc]
    def __init__(self, callback: Any, loop: asyncio.AbstractEventLoop, subscribed_items: list[str]) -> None:
...
class LightstreamerClientService:
    ...
    async def connect(self) -> None:
        ...
        sub = Subscription(mode="MERGE",
                                    items=IDENTIFIERS,
                                    fields=["TimeStamp", "Value"])
        # sub.setDataAdapter("QUOTE_ADAPTER")
        sub.setRequestedSnapshot("yes")
        sub.addListener(SubListener(callback=self.callback, loop=loop, subscribed_items=SUBS))
