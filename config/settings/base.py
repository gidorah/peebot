...
def _sentry_before_send(event: Event, _hint: dict[str, Any]) -> Event | None:
    """
    Filter Sentry events to drop those originating from health or readiness endpoint requests.
    
    Parameters:
        event (Event): The Sentry event payload.
        _hint (dict[str, Any]): SDK-provided hint information (ignored by this function).
    
    Returns:
        Event | None: `None` if the event corresponds to a request whose URL ends with `/healthz` or `/readyz` (drop the event), otherwise the original `event`.
    """
